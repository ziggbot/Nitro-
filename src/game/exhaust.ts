import Phaser from 'phaser';
import type { FuelDef } from '../config/fuels';
import type { CarSim } from './CarSim';

/**
 * Per-car exhaust effects styled by fuel type. A light idle emission
 * while driving makes every car's type readable at a glance; boosting
 * or overdrive switches to a dedicated afterburner emitter whose
 * particles fly ~150% farther (faster + longer-lived).
 */
export class ExhaustFx {
  private jet: Phaser.GameObjects.Particles.ParticleEmitter;
  private jetBoost: Phaser.GameObjects.Particles.ParticleEmitter;
  private smoke?: Phaser.GameObjects.Particles.ParticleEmitter;

  constructor(scene: Phaser.Scene, car: CarSim, fuel: FuelDef) {
    const electric = fuel.flicker;
    const rocket = fuel.smoke; // gas = rocket-style jet plume
    const tints = fuel.exhaustTints;
    const spread = rocket ? 7 : electric ? 24 : 15;

    const makeJet = (lengthMult: number): Phaser.GameObjects.Particles.ParticleEmitter => {
      const base = rocket ? { min: 260, max: 400 } : electric ? { min: 120, max: 260 } : { min: 80, max: 190 };
      const life = rocket ? { min: 220, max: 460 } : electric ? { min: 70, max: 190 } : { min: 150, max: 330 };
      const emitter = scene.add.particles(0, 0, 'dot', {
        speed: { min: base.min * lengthMult, max: base.max * lengthMult },
        angle: {
          onEmit: () => Phaser.Math.RadToDeg(car.heading + Math.PI) + (Math.random() * spread * 2 - spread),
        },
        x: { min: -4, max: 4 },
        y: { min: -4, max: 4 },
        scale: { start: (rocket ? 1.5 : electric ? 1.2 : 1.7) * (lengthMult > 1 ? 1.15 : 1), end: rocket ? 0.1 : 0 },
        alpha: { start: 0.95, end: 0 },
        lifespan: { min: life.min * lengthMult, max: life.max * lengthMult },
        frequency: 16,
        quantity: rocket ? 3 : 2,
        tint: { onEmit: () => tints[Math.floor(Math.random() * tints.length)] },
        blendMode: Phaser.BlendModes.ADD,
        emitting: false,
      });
      emitter.setDepth(9);
      return emitter;
    };

    // Cruise jet + a 150%-longer afterburner (speed × lifespan ≈ 2.5×).
    this.jet = makeJet(1);
    this.jetBoost = makeJet(1.6);

    if (fuel.smoke) {
      this.smoke = scene.add.particles(0, 0, 'orb', {
        speed: { min: 15, max: 55 },
        angle: { onEmit: () => Phaser.Math.RadToDeg(car.heading + Math.PI) + (Math.random() * 50 - 25) },
        scale: { start: 0.5, end: 1.5 },
        alpha: { start: 0.22, end: 0 },
        lifespan: { min: 450, max: 900 },
        frequency: 45,
        quantity: 1,
        tint: 0x9a9aa2,
        blendMode: Phaser.BlendModes.NORMAL,
        emitting: false,
      });
      this.smoke.setDepth(8);
    }
  }

  update(car: CarSim, rearOffset: number): void {
    const hot = car.alive && (car.boosting || car.overdriveTimer > 0);
    const idle = car.alive && Math.abs(car.speed) > 70;
    const rx = car.x - Math.cos(car.heading) * rearOffset;
    const ry = car.y - Math.sin(car.heading) * rearOffset;

    this.jet.setPosition(rx, ry);
    this.jet.emitting = idle && !hot;
    this.jet.frequency = 55;

    this.jetBoost.setPosition(rx, ry);
    this.jetBoost.emitting = hot;
    this.jetBoost.frequency = 12;

    if (this.smoke) {
      this.smoke.setPosition(rx, ry);
      this.smoke.emitting = hot || idle;
      this.smoke.frequency = hot ? 24 : 60;
    }
  }
}
